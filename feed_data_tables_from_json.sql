-- FUNCTION: odk_central.feed_data_tables_from_json(text, text, text, text)

-- DROP FUNCTION IF EXISTS odk_central.feed_data_tables_from_json(text, text, text, text);

CREATE OR REPLACE FUNCTION odk_central.feed_data_tables_from_json(
	schema_name text,
	table_name text,
	unique_column text,
	columns_to_keep_as_json text)
    RETURNS void
    LANGUAGE 'plpgsql'
    COST 100
...
EXECUTE format('SET search_path=odk_central,public;
	...
	WITH RECURSIVE doc_key_and_value_recursive(data_id, key, value) AS (
	  ...
	  FROM doc_key_and_value_recursive,
		json_each(CASE 
		  WHEN json_typeof(doc_key_and_value_recursive.value) <> ''object'' OR key = ANY(string_to_array('''||columns_to_keep_as_json||''','','')) THEN ''{}'' :: JSON
		  ELSE doc_key_and_value_recursive.value
		END) AS t
	)SELECT data_id, key, value FROM doc_key_and_value_recursive WHERE json_typeof(value) <> ''object'' OR key = ANY(string_to_array('''||columns_to_keep_as_json||''','','')) ORDER BY 2,1;'
);
...
ALTER FUNCTION odk_central.feed_data_tables_from_json(text, text, text, text)
    OWNER TO dba;
